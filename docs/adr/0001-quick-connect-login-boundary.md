...
Quick Connect authenticates JellyPilot to a known single Jellyfin Server URL from the Login screen. It does not discover servers, select between servers, or provide in-place account switching from the Operations Console, because this keeps the first Quick Connect slice aligned with the existing single-session app model while leaving richer server and account management as separate future work.

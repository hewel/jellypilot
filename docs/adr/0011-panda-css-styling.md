...
JellyPilot uses Panda CSS with owner-local `.styles.ts` modules, semantic tokens, and Solid-native class composition.
...
- Application styling is Panda-only. Legacy `.css.ts`, vanilla-extract, Sprinkles, Recipes, UnoCSS, and authored `--jellypilot-*` variables are not part of the active architecture.
...
- The final cutover removed vanilla-extract packages, Sprinkles, Recipes, the variable bridge, and every `.css.ts` module.

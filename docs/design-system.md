...
### Embedded Player Synchronization

Accepted native target: the [Paper embedded-player reference](https://app.paper.design/file/01M1XG9QCM2M58ENY2ZVA2YTWA/1-0/A4P-1). Implementation delivered; human visual acceptance is pending. Paper remains unchanged. This subsection owns the shared embedded-video chrome in normal, fullscreen, and Control-Only playback; the home/footer Player Bar and External MPV controller retain their existing presentation.

- Cinema chrome stays dark in either app theme. Use a 160px top scrim and 340px bottom scrim, with controls floating directly on the bottom gradient rather than inside a filled card. The 36px horizontal inset becomes 12px on narrow surfaces. Scoped transparency, neutral translucent edges, timeline glow, and the compact green play-method pill live in `widgets/embedded_player.rs`; they do not change global palette, font, shadow, or opacity rules.
- Back and Information occupy opposite top corners; either corner reveals the top chrome independently of the bottom transport. Framed icon controls retain at least 40px native hit targets, with a 44px primary play/pause control. The permanent volume slider is 84px wide. Metadata and controls reflow before the tools can overflow, while the wide layout keeps play/pause centered.
- The timeline uses a 4px rail, genuine buffered ranges, indigo gradient progress, and a persistent 14px outlined knob. Show elapsed time and remaining / total time. Hover is **time-only**: no preview frames, chapter labels, invented chapter markers, or sample buffer data. Preserve drag previews, release-to-seek, keyboard adjustment, and busy-input guards.
- Episode metadata opens a 320px queue with real 56×32 thumbnails, a highlighted current episode, and a scrollable list capped at 186px. Image demand belongs only to the visible queue viewport; closing or covering the queue retires its image demand. Overflow fades follow measured scroll geometry. Preserve manual scroll state across same-list media changes and temporary Settings coverage; discard stale geometry when the list changes or the popup actually closes.
- Information is a native, right-aligned popover capped at 380px and constrained to the window, using the reference's 16px radius, `#191A21CC` fill, neutral edge, and raised shadow. Its filename/title header, actual Direct Play / Direct Stream badge, and Close action precede File, Display, Video, and Audio groups. The whole panel scrolls in short windows so Close remains reachable.
- Populate Information from real MPV properties. Unavailable values remain explicit, never fabricated zeros. Derive the display filename from the raw playback path, separating URL components before decoding so credentials, query parameters, and fragments cannot appear in the filename. GPU timing is the sum of MPV GPU passes (last / average / per-pass peak), **not** total JellyPilot frame time or a whole-frame peak. Source color/HDR metadata is not a claim about display HDR output.
- Full statistics are sampled only while Information is open; buffer-only sampling is limited to visible controls. Closing, hiding, or replacing playback cancels retired observation demand and rejects late replies. Information and the existing MPV stats shortcut are independent entry points; retain the shortcut's MPV OSD behavior.
- Keep existing auto-hide, captured-input handling, adjacent playback, track selection, and intro/outro skip policy, including prompt dismissal. Do not change External MPV or footer-bar behavior to match this embedded-only reference.

Human acceptance: check normal, fullscreen, and Control-Only embedded playback in both app themes and languages; wide/narrow layout and long titles/timestamps; both top-corner reveal zones; seek hover/drag/release and permanent volume; opening/closing Information and the independent MPV stats shortcut; unavailable metrics; queue thumbnails/current-row reveal, scrolling, short-season replacement, and Settings cover/uncover without losing manual scroll position; skip prompt action/dismissal; unchanged external controller and footer bar. Agents use code-level checks and the headless native smoke gate, not desktop screenshots or injected input.
...
- Settings uses the scoped [2026-09-13 Settings synchronization](#2026-09-13-settings-synchronization) below: a floating panel over the existing dimmed blurred backdrop, with 208px section navigation and `primaryContainer` / `secondary` selection colors.
...
### 2026-09-13 Settings Synchronization
...
- Boolean preferences use the shared 40×22 switch with an 18px knob and 2px inset, not On/Off buttons. On uses `primary` / `onPrimary`; off uses `surfaceContainerHighest`, with `onPrimary` in dark mode and the readable `onControl` adaptation in light mode. Preserve disabled styling, keyboard focus, and a 40px-high hit target.
- Keep both themes and all UI languages through existing tokens and localization. Settings-only Catalog styles must not restyle the Sidebar Account Popover, add-account dialog, or confirmation dialogs; the explicitly app-wide switch migration also covers the sign-out confirmation's delete-Watchlist preference.

Human acceptance: check all eight sections in both themes; wide, short, and 480px Control-Only windows; unchanged left-navigation/right-content organization during resizing; visible rightmost controls without horizontal scrolling; reachable navigation and Close during scrolling; localized long labels; visible switch positions/focus; field Save/Enter and error feedback; subtitle ordering; shortcut capture/Esc; account switching and sign-out confirmation; diagnostic menus/export; backdrop dismissal in both App Modes and nested-dialog dismissal priority.
...
- The upward queue chevron is shown only when an episode queue is available; movies have no queue chevron. Busy playback can temporarily disable queue activation without removing an otherwise available episode affordance.
...
- Left/Right seek five seconds, Up/Down change volume five percentage points within 0–100%, and held arrows repeat. F toggles fullscreen without repeating; Esc leaves fullscreen after menu/modal priority. Space or a video click in an already active window toggles pause; no double-click action. Browsing and captured input do not trigger these player keys.
- Click-to-activate background presses do not toggle playback; explicit play/pause controls, seek, and volume remain operable on that same first click. Focus bookkeeping belongs to the stable window view, survives player mounting, and observes captured menu input in event order. No timed input lockout is used. iced does not expose activation-origin metadata: keyboard/taskbar activation followed by a background click without intervening distinct pointer movement or a key event can also consume one click conservatively.

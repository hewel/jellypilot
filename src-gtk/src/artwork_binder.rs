...
pub(crate) struct ArtworkSlot(u64);

#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub(crate) enum ArtworkSurface {
  Home,
  Browse,
  Detail,
  PlayerBar,
...
pub(crate) enum ArtworkSettlement {
  Apply,
  Drop,
}
...
#[derive(Default)]
pub(crate) struct ArtworkBinder {
  next_slot: u64,
  home_epoch: u64,
  browse_epoch: u64,
  detail_epoch: u64,
...
impl ArtworkBinder {
  ...
  pub(crate) fn begin_view(&mut self, surface: ArtworkSurface) {
  ...
  pub(crate) fn bind(&mut self, surface: ArtworkSurface) -> ArtworkSlot {
  ...
  pub(crate) fn bind_player_bar(&mut self) -> ArtworkSlot {
  ...
  fn bump(&mut self, surface: ArtworkSurface) {
    match surface {
      ...
      ArtworkSurface::PlayerBar => self.player_bar_epoch = self.player_bar_epoch.saturating_add(1),
    }

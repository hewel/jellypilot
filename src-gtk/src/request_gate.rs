...
pub(crate) struct DetailAuxToken {
  session: SessionToken,
  kind: DetailAuxKind,
  sequence: u64,
  item_id: String,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct ImageCacheToken(u64);
...
impl RequestGate {
  ...
  pub(crate) fn cancel_detail_loads(&mut self) {
    self.detail = None;
  }
  ...
  pub(crate) fn invalidate_detail_aux(&mut self, kind: DetailAuxKind) {
  ...
  #[must_use]
  pub(crate) const fn current_session(&self) -> SessionToken {
    SessionToken(self.session)
  }

  #[must_use]
  pub(crate) const fn is_current_session(&self, token: SessionToken) -> bool {
    token.0 == self.session
  }

  /// Records the item the current detail view presents.
  pub(crate) fn set_detail_item(&mut self, item_id: Option<String>) {
    self.detail_item = item_id;
  }

  /// Mints an aux token for the current detail item; None when no detail item is set.
  pub(crate) fn begin_detail_aux(&mut self, kind: DetailAuxKind) -> Option<DetailAuxToken> {
  ...
  #[must_use]
  pub(crate) fn finish_detail_aux(&mut self, token: DetailAuxToken) -> bool {
  ...
  pub(crate) fn begin_remote(&mut self) -> RemoteToken {
  ...
  #[must_use]
  pub(crate) const fn is_current_remote(&self, token: RemoteToken) -> bool {
    token.0 == self.remote
  }

  pub(crate) fn begin_remote_play(&mut self) -> RemotePlayToken {
  ...
  #[must_use]
  pub(crate) const fn is_current_remote_play(&self, token: RemotePlayToken) -> bool {
    token.remote.0 == self.remote && token.sequence == self.remote_play
  }

  pub(crate) fn begin_image_cache(&mut self) -> ImageCacheToken {
  ...
  #[must_use]
  pub(crate) fn finish_image_cache(&mut self, token: ImageCacheToken) -> bool {

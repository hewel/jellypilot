...
use jellypilot_media_server::{
    GeneralCommand, JellyfinError, PlayRequest, PlaystateRequest, WsMessage,
};
        ...
        loop {
            ...
            *connected.write() = true;
            ...
            if has_connected {
                ...
                if Self::send_event(&event_tx, JellyfinWebSocketEvent::Connected, &cancel_token)
                    .await
                {
                    break;
                }
...
mod tests {
    ...
    async fn expect_sessions_start(stream: &mut WebSocketStream<TcpStream>) {
        ...
        assert_eq!(value["Data"], "1000,1000");
    }

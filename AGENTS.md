...
- **Path aliases**: In `src/**`, import through the tsconfig aliases (`@components/*`, `@bindings`, `@styled-system/*`, `~effects/*`, `~utils/*`) instead of cross-directory relative paths. Same-directory imports (e.g. `./Component.styles`) stay relative. `tests/**` use relative imports — Rstest only aliases `@styled-system`.
